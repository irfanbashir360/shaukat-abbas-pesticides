    ...
    <div className="sap-page" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div className="sap-card" style={{ overflow: 'hidden' }}>
        <table className="sap-table">
          <thead>
            <tr>
              <th>Name</th><th>Unit</th><th>Price / Unit</th><th>Stock</th><th>Status</th><th></th>
            </tr>
          </thead>
          <tbody>
            {products.map(p => (
              <tr key={p.id}>
                ...
                <td>PKR {p.price_per_unit.toLocaleString()}</td>
      ...
      {modal && (
        <Modal title={modal === 'add' ? 'Add Product' : 'Edit Product'} onClose={closeModal}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '20px' }}>
            ...
            <button className="sap-btn sap-btn-primary" onClick={handleSave}>Save Product</button>
          </div>
        </Modal>
      )}
    </div>
